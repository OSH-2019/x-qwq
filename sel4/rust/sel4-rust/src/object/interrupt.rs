...
extern "C" {
    ...
    fn Arch_decodeIRQControlInvocation(invLabel: u64, length: u64, srcSlot: *mut cte_t,
                                       excaps: extra_caps_t, buffer: *mut u64) -> u64;
...
decodeIRQControlInvocation(invLabel: u64, length: u64,
                           srcSlot: *mut cte_t, excaps: extra_caps_t,
                           buffer: *mut u64) -> u64 {
    if invLabel == invocation_label::IRQIssueIRQHandler as u64 {
        ...
        if lu_ret.status != 0u64 {
            userError!("Target slot for new IRQ Handler cap invalid: cap %lu, IRQ %u.", 
                getExtraCPtr(buffer, 0), irq as i32);
...
#[no_mangle]
pub unsafe extern "C" fn
invokeIRQControl(irq: u8, handlerSlot: *mut cte_t, controlSlot: *mut cte_t) -> u64 {
    setIRQState(irq_state::IRQSignal as u64, irq);
    cteInsert(cap_irq_handler_cap_new(irq as u64), controlSlot, handlerSlot);
    0u64
}

#[no_mangle]
pub unsafe extern "C" fn
decodeIRQHandlerInvocation(invLabel: u64, irq: u64, excaps: extra_caps_t) -> u64 {
